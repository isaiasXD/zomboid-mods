...
function KnownAndCollectedRenderer.call(eventData, drawingContext, renderInstructions, instructionCount, playerObj)
    ...
    for r=1,instructionCount do
        ...
        if not hidden then
            ...
            -- INICIO DE SECCIÓN CORREGIDA PARA B42.15.2
            local javaObject = drawingContext.javaObject
            
            if javaObject then
                -- Verificación de seguridad para icono de recolección
                if unCollected and collectedTex[index] then
                ...
                -- Verificación de seguridad para texturas de estado (Evita el error nil en línea 157)
                if bottomRightTex and bottomRightTex[index] then
